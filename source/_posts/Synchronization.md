...
同样以一个同步坐标为例，在游戏开始的时候，客户端A和DS服务器有一个相同的坐标状态A(0,0)，客户端A在接受到玩家的输入后，执行游戏逻辑，运算后会将A的坐标改变为A(1,0)，在这之后，客户端将运算的结果(1,0)和用户的输入都发送给DS，DS接受到消息后执行相同的游戏逻辑，运算后得到的结果也为(1,0)，客户端和DS达成同步。

不过，客户端在接受用户的第二次收入后，将坐标更新为(2,0)，DS验算的结果却将(1,0)改变为(1,1)，客户端A的状态与DS不一样，DS就会强行将客户端A的的坐标修正为(1,1)，也就是拉扯。

相比帧同步所有的客户端逻辑帧内拥有相同的状态，状态同步则是通过每帧传输所有的状态信息来保证客户端和DS的同步，不需要预表现的属性，比如血量，攻击力，无论什么时候DS和客户端都拥有相同的状态。需要预表现的属性，比如位置信息，在停止相关操作后，也能达迅速达成客户端和DS的同步。
